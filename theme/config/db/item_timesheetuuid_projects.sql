CREATE TABLE `SITE_DB`.`item_timesheetuuid_projects` (
  ...
  CONSTRAINT `item_timesheetuuid_projects_ibfk_1` FOREIGN KEY (`item_timesheetuuid_id`) REFERENCES `SITE_DB`.`item_timesheetuuid` (`item_id`) ON DELETE CASCADE ON UPDATE CASCADE,
